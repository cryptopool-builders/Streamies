    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zSTRMS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSTRMS seed used to mint this zSTRMS is not the same as currently hold in the wallet";
            else
