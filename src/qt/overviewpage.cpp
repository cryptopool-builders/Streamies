...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeStreamiesAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeStreamiesAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeStreamiesAmount = strAnonymizeStreamiesAmount.remove(strAnonymizeStreamiesAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStreamiesAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeStreamiesAmount * COIN) nMaxToAnonymize = nAnonymizeStreamiesAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeStreamiesAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeStreamiesAmount));
        strAnonymizeStreamiesAmount = strAnonymizeStreamiesAmount.remove(strAnonymizeStreamiesAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStreamiesAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeStreamiesAmount == 0) {
