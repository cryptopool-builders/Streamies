STRMS Core version *3.1.0* is now available from:  <https://github.com/streamies-project/streamies/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/streamies-project/streamies/issues>


Mandatory Update
==============

STRMS Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/STRMS-Qt (on Mac) or streamiesd/streamies-qt (on Linux).


Compatibility
==============

STRMS Core is extensively tested on multiple operating systems using
...
STRMS Core should also work on most other Unix-like systems but is not
...
zPIV Staking is here! zPIV staking will be activated on the morning of the 8th of May 2018. With the release of zPIV staking, there are effectively 2 versions of zPIV, zPIV minted on the 3.0.6 STRMS wallet or lower, and zPIV minted on STRMS wallet version or higher. New features in this release will require the use of zPIV v2, zPIV minted on this wallet release 3.1.0 or later. If you currently hold zPIV v1 and wish to take advantage of zPIV staking and deterministic zPIV, you will need to spend the zPIV v1 to yourself and remint zPIV v2.
...
To encourage the use of zPIV and increase the STRMS zerocoin anonymity set, the STRMS payment schedule has been changed to the following:
...
The visual layout of the STRMS Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zPIV minting feature. This is enabled by default and the enablezeromint=0 setting in the streamies.conf file will overwrite the GUI option.
...
### Summary
 ...
 The STRMS wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the streamies.conf file, found in the STRMS data directory.
...
STRMS Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The STRMS client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running streamiesd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the STRMS support group, which often times sees users say "I didn't spend that zPIV". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the STRMS masternodes, the monthly budget available to be utilised has been increased to 42,000 PIV / month. This PIV only has the opportunity to be raised once per month (paid to winning proposals) with any unused PIV not created by the blockchain.

### Budget Finalization Fee

The STRMS finalization fee for successful proposals has now been reduced, this fee is now 5 PIV down from 50 PIV. The total fee outlay for a successful proposal is now a total of 55 PIV.
...
-	Fixed a  bug that would not start masternodes from the STRMS-Qt masternodes tab
-	Updated STRMS-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and STRMS's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zPIV Control dialog in the QT wallet, a user is able to see zPIV marked as version 1 or 2.
...
STRMS's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which STRMS used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to STRMS using the bug bounty program. The modulus is now correctly imported and STRMS's accumulators have been changed to use the new proper modulus.
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault streamies-cli getinfo while loading block index (Mrs-X)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/streamies-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
